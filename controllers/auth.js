    ...
    try {
        ...
        connection.query('SELECT * FROM user WHERE email = ?', [email], async (error, results) => {
            ...
            if(error) {
                ...
                const id = results[0].id;                // const token = jwd.sign({id}, process.env.JWT_SECRET, {
                //     expiresIn: process.env.JWT_EXPIRES_IN
                // });
                //
                // console.log("The token is: " + token);                // const cookieOptions = {
                //     expires: new Date(
...
exports.register = (req, res) => {
    ...
    connection.query("SELECT email FROM user WHERE email = ?", [email], async (error, results) => {
        ...
        } else {            let hashedPassword = await bcrypt.hash(password, 10);            connection.query('INSERT INTO user SET ?', {
            username: name,
            email: email,
...
//        "WHERE user_id = ?",[req.session.user_id],async (error,results) =>{}
